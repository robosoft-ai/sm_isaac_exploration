...
if [ $ARCH == "aarch64" ]; then
    ...
    docker build -t pibgeus/smacc2_isaac_exploration -f src/sm_isaac_exploration/docker/Dockerfile_nvidia_arm64 --build-arg architecture_=$ARCH --progress=plain .
    sudo umount src/root_mount/
else
    docker build -t pibgeus/smacc2_isaac_exploration -f src/sm_isaac_exploration/docker/Dockerfile --build-arg architecture_=$ARCH --progress=plain .
fi
