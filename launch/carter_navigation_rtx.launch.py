...
def generate_launch_description():
    ...
    map_dir = LaunchConfiguration(
        ...
        default=os.path.join(
            get_package_share_directory("sm_isaac_exploration"), "maps", "improved_carter_warehouse_navigation.yaml"
    ...
    param_dir = LaunchConfiguration(
        "params_file",
        default=os.path.join(
            get_package_share_directory("sm_isaac_exploration"), "config", "rtx_carter_navigation_params.yaml"
        ),
    )
    ...
    isaac_exploration_launch_dir = os.path.join(get_package_share_directory("sm_isaac_exploration"), "launch")

    # rviz_config_dir = os.path.join(get_package_share_directory("carter_navigation"), "rviz2", "carter_navigation.rviz")
    rviz_config_dir = os.path.join(get_package_share_directory("sm_isaac_exploration"), "config", "rtx_rviz_conf.rviz")
    ...
    return LaunchDescription(
        [
            ...
            get_package_share_directory("sm_isaac_exploration"), "config", "navigation_tree.xml"
                )
                ,
                description="Full path to the behavior tree xml file to use"
            ),
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(os.path.join(isaac_exploration_launch_dir, "rviz_launch.py")),
                launch_arguments={"namespace": "", "use_namespace": "False", "rviz_config": rviz_config_dir}.items(),
            ),
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource([isaac_exploration_launch_dir, "/bringup_launch.py"]),
